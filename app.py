...
def load_file(uploaded_file):
    ...
    try:
        ...
        if file_extension == '.csv':
            try:
                # Read bytes once (avoid consumed stream across retries)
                raw = uploaded_file.getvalue()
                if not raw or len(raw) == 0:
                    st.error("El archivo CSV está vacío (0 bytes)")
                    return None

                # Try common encodings (Excel often uses UTF-8 with BOM or Latin-1 variants)
                encodings = ['utf-8-sig', 'utf-8', 'latin1', 'iso-8859-1', 'cp1252']
                last_err = None

                for encoding in encodings:
                    try:
                        bio = io.BytesIO(raw)  # fresh stream each attempt
                        df = pd.read_csv(
                            bio,
                            encoding=encoding,
                            sep=None,        # infer separator: comma/semicolon/tab
                            engine='python'  # required for sep=None
                        )

                        # If separator inference fails and we end up with 1 column, try ';'
                        if df is not None and df.shape[1] == 1:
                            bio2 = io.BytesIO(raw)
                            df2 = pd.read_csv(bio2, encoding=encoding, sep=';')
                            if df2.shape[1] > 1:
                                df = df2

                        if df is None:
                            last_err = ValueError("No se pudo parsear el CSV (df=None)")
                            continue
                        if df.empty:
                            last_err = ValueError("El archivo CSV está vacío")
                            continue
                ...
                if last_err is not None:
                    st.caption(f"Último error: {last_err}")
                return None
